...
while True:
    testid=pyui.chooseinlist(xi['data'],color('考试列表',bg=green),fm='{[examName]}')
    ...
    clear()
    tmp=pyui.chooseinlist(['成绩趋势(不可用)','班级成绩分布(不可用)','班级排名','错题(不可用)','原卷'],color(testname,bg=green))
    clear()
    [None,None,bjpm,None,yj][tmp](d)